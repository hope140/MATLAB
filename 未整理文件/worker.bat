@echo off
REM Wrapper around matlab.bat for MATLAB Parallel Server
...
FOR /F "usebackq delims=" %%I IN (`""!MATLAB_BIN_DIR!MATLAB.exe"" -query -parallelserver -noFigureWindows %*`) DO (
    %%I>NUL 2>NUL
)
...
"!MATLAB_BIN_DIR!%MATLAB_ARCH%\MATLAB.exe" -parallelserver -noFigureWindows -r "%MATLAB_FUNCTION_TO_CALL%" %1 %2 %3 %4 %5 %6 %7 %8 %9
